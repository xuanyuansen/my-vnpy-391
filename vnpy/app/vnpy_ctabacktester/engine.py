...
class BacktesterEngine(BaseEngine):
    ...
    def load_strategy_class(self) -> None:
        ...
        print("path1 is {}".format(
            path1
        ))
        self.load_strategy_class_from_folder(path1, "vnpy_ctastrategy.strategies")

        path2: Path = Path.cwd().joinpath("strategies")
        print("path2 is {}".format(
            path2
        ))
        self.load_strategy_class_from_folder(path2, "strategies")
        script_path = Path(os.path.abspath(__file__)).parent
        print(script_path.joinpath("strategies"))
        self.load_strategy_class_from_folder(script_path.joinpath("strategies"), "strategies")
